...
public class UserService {
    ...
    public UserService(UserMapper userMapper, PasswordEncoder passwordEncoder, JwtUtil jwtUtil,
                       EmailService emailService, UserFollowMapper userFollowMapper,
    ...
    public void register(RegisterRequest req) {
        ...
        if (req.getEmail() != null && !req.getEmail().isBlank()) {
            ...
            if (emailCount > 0) throw new BusinessException("该邮箱已被其他账号绑定");
        }
        User user = new User();
    ...
    public LoginResponse login(LoginRequest req) {
        ...
        if (req.getUsername().contains("@")) {
            ...
            if (user == null) throw new BusinessException("该邮箱未注册或密码错误");
        } else {
            user = userMapper.selectOne(
                    new LambdaQueryWrapper<User>().eq(User::getUsername, req.getUsername()));
            if (user == null) throw new BusinessException("用户名或密码错误");
        }
        if ("禁用".equals(user.getStatus())) throw new BusinessException(403, "账号已被禁用，请联系管理员");
    ...
    public PageResult<UserProfileVO> searchUsers(String keyword, int page, int size) {
        var userPage = userMapper.selectPage(
                new Page<>(page, size),
                new LambdaQueryWrapper<User>()
                        .like(keyword != null && !keyword.isBlank(), User::getUsername, keyword)
                        .eq(User::getStatus, "正常")
                        .orderByDesc(User::getCreateTime));
        List<UserProfileVO> list = userPage.getRecords().stream().map(u -> {
            UserProfileVO vo = new UserProfileVO();
            vo.setId(u.getId());
            vo.setUsername(u.getUsername());
            vo.setAvatar(u.getAvatar());
            vo.setBio(u.getBio());
            vo.setFollowersCount(userFollowMapper.countFollowers(u.getId()));
            return vo;
        }).collect(Collectors.toList());
        return PageResult.of(userPage.getTotal(), list, page, size);
    }
    ...
    public void sendEmailCode(String email, String purpose, Long userId) {
        ...
        if (count > 0) throw new BusinessException("该邮箱已被其他账号绑定");
        emailService.sendCode(email, purpose);
    }

    public void bindEmail(Long userId, String email, String code) {
        if (!emailService.verifyCode(email, code, "bind")) throw new BusinessException("验证码错误或已过期");
        Long count = userMapper.selectCount(new LambdaQueryWrapper<User>()
                .eq(User::getEmail, email).ne(User::getId, userId));
        if (count > 0) throw new BusinessException("该邮箱已被其他账号绑定");
        User user = new User();
    ...
    public void changeEmail(Long userId, String newEmail, String code) {
        if (!emailService.verifyCode(newEmail, code, "change")) throw new BusinessException("验证码错误或已过期");
        Long count = userMapper.selectCount(new LambdaQueryWrapper<User>()
    ...
    public void sendPasswordResetCode(String email) {
        ...
        if (count == 0) throw new BusinessException("该邮箱未绑定任何账号");
        emailService.sendCode(email, "reset");
    }

    public void resetPassword(String email, String code, String newPassword) {
        if (!emailService.verifyCode(email, code, "reset")) throw new BusinessException("验证码错误或已过期");
    ...
    public void updateUsername(Long userId, String newUsername) {
        Long count = userMapper.selectCount(new LambdaQueryWrapper<User>()
                .eq(User::getUsername, newUsername).ne(User::getId, userId));
    ...
    public void changePassword(Long userId, String oldPassword, String newPassword) {
        User user = userMapper.selectById(userId);
        if (user == null) throw new BusinessException(404, "用户不存在");
        if (!passwordEncoder.matches(oldPassword, user.getPassword())) throw new BusinessException("当前密码错误");
        user.setPassword(passwordEncoder.encode(newPassword));
        userMapper.updateById(user);
    }
    ...
    public void deleteAccount(Long userId, String password, String code) {
        ...
        if (user.getEmail() != null && !user.getEmail().isBlank()) {
            if (code == null || code.isBlank()) throw new BusinessException("请填写邮箱验证码");
            if (!emailService.verifyCode(user.getEmail(), code, "delete")) throw new BusinessException("验证码错误或已过期");
        }
